...
export function createSimulatorUI() {
  ...
  function seekMedia(ratio) {
    if (state.media?.type !== 'video' || !Number.isFinite(state.media.element.duration)) return;
    state.media.element.currentTime = Math.min(state.media.element.duration, Math.max(0, ratio) * state.media.element.duration);
  }
  function mediaState() {
    const element = state.media?.type === 'video' ? state.media.element : null;
    return {
      isVideo: Boolean(element),
  ...
  render(); return { textures: canvases, beginUnlock, previewMedia, playMedia, pauseMedia, seekMedia, mediaState, lockScreen, setDarkMode, update, tap, get page() { return state.page; }, get hasVideo() { return state.page === 'media' && state.media?.type === 'video'; } };
}
